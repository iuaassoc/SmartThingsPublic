...
metadata {
	definition (
		name: "Sept 17th - Monoprice Z-Wave Plus Door/Window Sensor Used as Water Sensor", 
		namespace: "Steve", 
...
def configure() {	
	...
	if (!device.currentValue("water")) {
		sendEvent(createEventMap("water", "wet", false))
	}
...
private handleContactEvent(event) {
	...
	if (val) {
		result << createEvent(createEventMap("water", val))
	}
	return result
}
